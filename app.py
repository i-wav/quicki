...
st.set_page_config(page_title="Quicki UPI Fraud Detection", layout="wide")
# ---- Center Tab Labels ----
st.markdown("""
<style>
...
st.title("Quicki: UPI Fraud Detection")

# ---------------- Tabs ----------------
tab1, tab2, tab3 = st.tabs(["🏠 Home", "🔍 Fraud Detection", "📊 Model Performance"])
...
with tab1:
    st.header("Welcome")
    st.write("""
    This system uses a trained **CatBoost Machine Learning model** to detect fraudulent  
    UPI transactions in real time.  
    Enter transaction details in the **Fraud Detection tab** to get predictions.
    """)
    ...
    - CatBoost model with 0.8696 ROC-AUC  
    - Streamlit-based lightweight deployment  
    - Uses behavioural indicators like  
      *Device Change, Night Transaction, Previous Fraud Counts*
    """)

# =====================================================================================
# TAB 2 — FRAUD DETECTION UI
# =====================================================================================
with tab2:
    st.header("Detect Fraudulent UPI Transaction")
...
with tab3:
    ...
    st.write("### CatBoost Fraud Detection Metrics")
    st.table({
        "Metric": ["ROC-AUC", "Precision (Fraud)", "Recall (Fraud)", "F1-Score"],
        "Value": ["0.8696", "0.49", "0.60", "0.54"]
    })

    st.write("### CatBoost vs XGBoost Comparison")
    st.table({
        "Model": ["CatBoost", "XGBoost"],
        "ROC-AUC": ["0.8696", "0.8443"],
        "Recall (Fraud)": ["0.60", "0.42"]
    })

    # st.info("More visualizations (ROC curve, Confusion Matrix) can be added here.")
